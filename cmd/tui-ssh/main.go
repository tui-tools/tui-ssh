// Command tui-ssh is a terminal UI for the machine's SSH server: the
// configuration sshd is actually running, who is logged in, what the
// authentication log says, the host keys, and the service behind them. It
// previews the exact command line of every change before running it. OpenSSH
// is the backend implemented today; the code is written against a generic
// interface so another server could follow.
package main
...
// /etc/tui-ssh/config.toml and ~/.config/tui-ssh/config.toml.
const toolName = "tui-ssh"
...
const backendName = "openssh"

// version is stamped by the release build (-ldflags "-X main.version=…").
var version = "dev"

// defaults declares the configuration keys tui-ssh understands. Only these are
// read from the environment (TUI_SSH_SUDO, …).
...
func parseFlags(args []string, out *os.File) (options, error) {
	...
	fs.BoolVar(&opts.demo, "demo", false,
		"run against a sample server, without touching the real one")
	...
	fs.Usage = func() {
		_, _ = fmt.Fprintf(out, "tui-ssh — a terminal UI for the machine's SSH server\n\n"+
			"Usage:\n  tui-ssh [flags]\n\nFlags:\n")
		fs.PrintDefaults()
		_, _ = fmt.Fprintf(out, "\nConfiguration is read from %s, then %s, "+
			"then TUI_SSH_* in the environment.\n",
...
func run(args []string) error {
	...
	// the backend needs the capability set: which spelling of a keyword this
	// OpenSSH accepts is a version question, and the answer comes from the
	// manifest.
...
func pickBackend(cfg config.Config, opts options,
	backendCompat compat.Result) (ssh.Backend, error) {
	if opts.demo {
		return openssh.NewFake(), nil
	}
	return openssh.NewReal(cfg.SudoPrefix(), backendCompat.Caps())
}
